...
public class CreateL2 extends BaseOperation<Context> implements
		Function<Context> {
	...
	public CreateL2(Fields fields, JobConf conf) {
		...
		try {
			...
			for (int i = 0; i < uri.length; i++) {
				if (fs.exists(new Path(uri[i]))) {
					FSDataInputStream inStream = fs.open(new Path(uri[i]));
